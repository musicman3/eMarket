...
if (\eMarket\Valid::inGET('search')) {
    ...
    $products = \eMarket\Pdo::getColAssoc("SELECT * FROM " . TABLE_PRODUCTS . " WHERE language=? AND parent_id=? AND status=1 ORDER BY id DESC", [lang('#lang_all')[0], \eMarket\Valid::inGET('category_id')]);
}
