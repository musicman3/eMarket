...
//$menu[$menu_market][2] = array('#', 'glyphicon glyphicon-hourglass', 'Ожидаемые', '', 'false');
$menu[$menu_market][2] = array('?route=settings', 'glyphicon glyphicon-cog', lang('title_settings_index'), '', 'false');

//ПРОДАЖИ
$level[$menu_sales] = array('#', lang('menu_sellings'), 'true');

$menu[$menu_sales][0] = array('?route=orders', 'glyphicon glyphicon-shopping-cart', lang('title_orders_index'), '', 'false');
//$menu[$menu_sales][1] = array('#', 'glyphicon glyphicon-import', lang('menu_refunds'), '', 'false');

//МАРКЕТИНГ
$level[$menu_marketing] = array('#', lang('menu_marketing'), 'true');
...
//$menu[$menu_count_linesparty][1] = array('#', 'glyphicon glyphicon-eye-open', 'Кто в онлайне', '', 'false');
...
for ($lng = 0; $lng < count(lang('#lang_all')); $lng++) {
    $submenu[$menu_help][1][$lng] = array(\eMarket\Settings::langCurrencyPath() . '&language=' . lang('#lang_all')[$lng], 'glyphicon glyphicon-triangle-right', lang('language_name', lang('#lang_all')[$lng]), '');
}
