...
for ($i = $start; $i < $finish; $i++) {
    if (isset($lines_cat[$i][0]) == TRUE) {
        ...
        for ($x = 0; $x < $count_lang; $x++) {
            $name_edit_temp[$x][$modal_id] = \eMarket\Pdo::selectPrepare("SELECT name FROM " . TABLE_CATEGORIES . " WHERE id=? and language=?", [$modal_id, lang('#lang_all')[$x]]);
        }
        $query = \eMarket\Pdo::getRow("SELECT logo, logo_general FROM " . TABLE_CATEGORIES . " WHERE id=?", [$modal_id]);
