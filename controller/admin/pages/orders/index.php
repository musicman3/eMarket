...
if (\eMarket\Valid::inGET('search')) {
    $lines = \eMarket\Pdo::getColAssoc("SELECT * FROM " . TABLE_ORDERS . " WHERE id LIKE? OR email LIKE? OR customer_data REGEXP? OR customer_data RLIKE? ORDER BY id DESC", [$search, $search, '["lastname": "' . \eMarket\Valid::inGET('search') . ']', '["firstname": "' . \eMarket\Valid::inGET('search') . ']']);
} else {
