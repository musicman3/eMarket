...
if ($VALID->inGET('add')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        $PDO->inPrepare("INSERT INTO " . TABLE_REGIONS . " SET id=?, country_id=?, name=?, language=?, region_code=?", [$id, $VALID->inGET('country_id'), $VALID->inGET($TITLE_DIR . '_' . $lang_all[$xl]), $lang_all[$xl], $VALID->inGET('region_code')]);
    }
}
...
if ($VALID->inGET('id_edit')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        // обновляем запись
        $PDO->inPrepare("UPDATE " . TABLE_REGIONS . " SET name=?, region_code=? WHERE id=? AND language=?", [$VALID->inGET('name_edit_' . $TITLE_DIR . '_' . $lang_all[$xl]), $VALID->inGET('region_code_edit'), $VALID->inGET('id_edit'), $lang_all[$xl]]);
    }
}
