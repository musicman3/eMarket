...
for ($i = $start; $i < $finish; $i++) {
    if (isset($lines[$i][0]) == TRUE) {
        ...
        for ($x = 0; $x < $count_lang; $x++) {
            $query_lang = \eMarket\Core\Pdo::getRow("SELECT name, code FROM " . TABLE_WEIGHT . " WHERE id=? and language=?", [$modal_id, lang('#lang_all')[$x]]);
            $name_edit_temp[$x][$modal_id] = $query_lang[0];
            $code_edit_temp[$x][$modal_id] = $query_lang[1];
        }
        
        $query = \eMarket\Core\Pdo::getRow("SELECT value_weight, default_weight FROM " . TABLE_WEIGHT . " WHERE id=?", [$modal_id]);
