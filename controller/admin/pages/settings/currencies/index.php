...
if ($VALID->inPOST('add')) {
    ...
    if ($id > 1 && $default_value != 0) {
        ...
        for ($x = 0; $x < $count_value_all; $x++) {
            \eMarket\Core\Pdo::inPrepare("UPDATE " . TABLE_CURRENCIES . " SET value=? WHERE id=? AND language=?", [($value_all[$x]['value'] / $VALID->inPOST('value_currencies')), $value_all[$x]['id'], $value_all[$x]['language']]);
        }
...
if ($VALID->inPOST('edit')) {
    ...
    if ($default_value != 0) {
        ...
        $count_value_all = count($value_all);
        for ($x = 0; $x < $count_value_all; $x++) {
            \eMarket\Core\Pdo::inPrepare("UPDATE " . TABLE_CURRENCIES . " SET value=? WHERE id=? AND language=?", [($value_all[$x]['value'] / $VALID->inPOST('value_currencies_edit')), $value_all[$x]['id'], $value_all[$x]['language']]);
        }

        for ($x = 0; $x < $LANG_COUNT; $x++) {
            // обновляем запись
            \eMarket\Core\Pdo::inPrepare("UPDATE " . TABLE_CURRENCIES . " SET name=?, code=?, iso_4217=?, value=?, default_value=?, symbol=?, symbol_position=?, decimal_places=?, last_updated=? WHERE id=? AND language=?", [$VALID->inPOST('name_currencies_edit_' . $x), $VALID->inPOST('code_currencies_edit_' . $x), $VALID->inPOST('iso_4217_currencies_edit'), 1, $default_value, $VALID->inPOST('symbol_currencies_edit'), $VALID->inPOST('symbol_position_currencies_edit'), $VALID->inPOST('decimal_places_currencies_edit'), date("Y-m-d H:i:s"), $VALID->inPOST('edit'), lang('#lang_all')[$x]]);
        }
    } else {
...
if ($VALID->inPOST('delete')) {
    ...
    // Удаляем
    \eMarket\Core\Pdo::inPrepare("DELETE FROM " . TABLE_CURRENCIES . " WHERE id=?", [$VALID->inPOST('delete')]);
    // Выводим сообщение об успехе
    $_SESSION['message'] = ['success', lang('action_completed_successfully')];
}
