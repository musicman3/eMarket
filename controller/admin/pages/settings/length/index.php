...
if (\eMarket\Valid::inPOST('edit')) {
    ...
    // Если есть установка по-умолчанию
    if (\eMarket\Valid::inPOST('default_length_edit')) {
        $default_length = 1;
    } else {
        $default_length = 0;
    }
    ...
    if ($default_length != 0) {
        ...
        $count_value_length_all = count($value_length_all);
        for ($x = 0; $x < $count_value_length_all; $x++) {
            \eMarket\Pdo::inPrepare("UPDATE " . TABLE_LENGTH . " SET value_length=? WHERE id=? AND language=?", [($value_length_all[$x]['value_length'] / \eMarket\Valid::inPOST('value_length_edit')), $value_length_all[$x]['id'], $value_length_all[$x]['language']]);
        }

        for ($x = 0; $x < $LANG_COUNT; $x++) {
            // обновляем запись
            \eMarket\Pdo::inPrepare("UPDATE " . TABLE_LENGTH . " SET name=?, code=?, value_length=?, default_length=? WHERE id=? AND language=?", [\eMarket\Valid::inPOST('name_length_edit_' . $x), \eMarket\Valid::inPOST('code_length_edit_' . $x), 1, $default_length, \eMarket\Valid::inPOST('edit'), lang('#lang_all')[$x]]);
        }
    } else {

        for ($x = 0; $x < $LANG_COUNT; $x++) {
            // обновляем запись
            \eMarket\Pdo::inPrepare("UPDATE " . TABLE_LENGTH . " SET name=?, code=?, value_length=?, default_length=? WHERE id=? AND language=?", [\eMarket\Valid::inPOST('name_length_edit_' . $x), \eMarket\Valid::inPOST('code_length_edit_' . $x), \eMarket\Valid::inPOST('value_length_edit'), $default_length, \eMarket\Valid::inPOST('edit'), lang('#lang_all')[$x]]);
        }
    }
    // Выводим сообщение об успехе
    $_SESSION['message'] = ['success', lang('action_completed_successfully')];
