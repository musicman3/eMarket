...
if ($VALID->inGET('add')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        $PDO->inPrepare("INSERT INTO " . TABLE_ZONES . " SET id=?, name=?, note=?, language=?", [$id, $VALID->inGET($SETTINGS->TitleDir() . '_' . $lang_all[$xl]), $VALID->inGET('note'), $lang_all[$xl]]);
    }
}
...
if ($VALID->inGET('id_edit')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        // обновляем запись
        $PDO->inPrepare("UPDATE " . TABLE_ZONES . " SET name=?, note=? WHERE id=? AND language=?", [$VALID->inGET('name_edit_' . $SETTINGS->TitleDir() . '_' . $lang_all[$xl]), $VALID->inGET('note'), $VALID->inGET('id_edit'), $lang_all[$xl]]);
    }
}
