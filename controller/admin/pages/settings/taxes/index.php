...
if ($VALID->inGET('add')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        $PDO->inPrepare("INSERT INTO " . TABLE_TAXES . " SET id=?, name=?, language=?, rate=?", [$id, $VALID->inGET($SETTINGS->TitleDir() . '_' . $lang_all[$xl]), $lang_all[$xl], $VALID->inGET('rate')]);
    }
}
...
if ($VALID->inGET('id_edit')) {
    ...
    for ($xl = 0; $xl < count($lang_all); $xl++) {
        // обновляем запись
        $PDO->inPrepare("UPDATE " . TABLE_TAXES . " SET name=?, rate=? WHERE id=? AND language=?", [$VALID->inGET('name_edit_' . $SETTINGS->TitleDir() . '_' . $lang_all[$xl]), $VALID->inGET('rate_edit'), $VALID->inGET('id_edit'), $lang_all[$xl]]);
    }
}
