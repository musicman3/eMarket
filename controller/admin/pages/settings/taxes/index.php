...
if (\eMarket\Valid::inPOST('edit')) {
    ...
    for ($x = 0; $x < $LANG_COUNT; $x++) {
        // обновляем запись
        \eMarket\Pdo::inPrepare("UPDATE " . TABLE_TAXES . " SET name=?, rate=? WHERE id=? AND language=?", [\eMarket\Valid::inPOST('name_taxes_' . $x), \eMarket\Valid::inPOST('rate_taxes'), \eMarket\Valid::inPOST('edit'), lang('#lang_all')[$x]]);
    }
