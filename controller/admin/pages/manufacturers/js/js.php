    ...
    <script type="text/javascript" language="javascript">
        $('#edit').on('show.bs.modal', function (event) {
            ...
            for (x = 0; x < logo_edit[modal_id].length; x++) {
                ...
                $('<span/>').html('<span class="file-upload" id="image_' + x + '"><div class="holder"><img src="/downloads/images/manufacturers/resize/' + image + '" class="thumbnail" height="60" /><div class="block"><button class="btn btn-primary btn-xs" type="button" name="delete_image_' + x + '" onclick="delete_image(\'' + image + '\', \'' + modal_id + '\', \'' + x + '\')"><span class="glyphicon glyphicon-trash"></span></button></div></div> </span>').appendTo('#logo-edit'); // Вставляем лого
            }
