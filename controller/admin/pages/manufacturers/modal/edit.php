...
for ($i = $start; $i < $finish; $i++) {
    if (isset($lines[$i][0]) == TRUE) {
        ...
        for ($x = 0; $x < $count_lang; $x++) {
            $name_edit_temp[$x][$modal_id] = \eMarket\Core\Pdo::selectPrepare("SELECT name FROM " . TABLE_MANUFACTURERS . " WHERE id=? and language=?", [$modal_id, lang('#lang_all')[$x]]);
        }
        
        $query = \eMarket\Core\Pdo::getRow("SELECT site, logo, logo_general FROM " . TABLE_MANUFACTURERS . " WHERE id=?", [$modal_id]);
