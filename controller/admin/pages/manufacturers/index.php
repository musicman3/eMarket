...
if ($VALID->inPOST('add')) {
    ...
    for ($x = 0; $x < $LANG_COUNT; $x++) {
        \eMarket\Core\Pdo::inPrepare("INSERT INTO " . TABLE_MANUFACTURERS . " SET id=?, name=?, language=?, site=?", [$id, $VALID->inPOST('name_manufacturers_' . $x), lang('#lang_all')[$x], $VALID->inPOST('site_manufacturers')]);
    }
...
if ($VALID->inPOST('edit')) {
    ...
    for ($x = 0; $x < $LANG_COUNT; $x++) {
        \eMarket\Core\Pdo::inPrepare("UPDATE " . TABLE_MANUFACTURERS . " SET name=?, site=? WHERE id=? AND language=?", [$VALID->inPOST('name_manufacturers_edit_' . $x), $VALID->inPOST('site_manufacturers_edit'), $VALID->inPOST('edit'), lang('#lang_all')[$x]]);
    }
...
if ($VALID->inPOST('delete')) {
    // Удаляем запись
    \eMarket\Core\Pdo::inPrepare("DELETE FROM " . TABLE_MANUFACTURERS . " WHERE id=?", [$VALID->inPOST('delete')]);
    // Выводим сообщение об успехе
    $_SESSION['message'] = ['success', lang('action_completed_successfully')];
}
