...
class Stock {
    ...
    public function selectData() {
        self::$currencies_all = \eMarket\Pdo::getColAssoc("SELECT name, default_value, id FROM " . TABLE_CURRENCIES . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$taxes_all = \eMarket\Pdo::getColAssoc("SELECT name, id FROM " . TABLE_TAXES . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$units_all = \eMarket\Pdo::getColAssoc("SELECT name, default_unit, id FROM " . TABLE_UNITS . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$length_all = \eMarket\Pdo::getColAssoc("SELECT name, default_length, id FROM " . TABLE_LENGTH . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$weight_all = \eMarket\Pdo::getColAssoc("SELECT name, default_weight, id FROM " . TABLE_WEIGHT . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$vendor_codes_all = \eMarket\Pdo::getColAssoc("SELECT name, default_vendor_code, id FROM " . TABLE_VENDOR_CODES . " WHERE language=?", [lang('#lang_all')[0]]);
        self::$manufacturers_all = \eMarket\Pdo::getColAssoc("SELECT name, id FROM " . TABLE_MANUFACTURERS . " WHERE language=?", [lang('#lang_all')[0]]);
    }
    ...
    public function preparedData() {
        ...
        if (self::$parent_id == 0) {
            ...
            self::$attributes_category = json_encode(\eMarket\Pdo::getColAssoc("SELECT attributes FROM " . TABLE_CATEGORIES . " WHERE id=? AND language=?", [self::$parent_id, lang('#lang_all')[0]])[0]['attributes']);
        }
    ...
    public function data() {
        ...
        if (\eMarket\Valid::inGET('search')) {
            ...
            self::$lines_cat = \eMarket\Func::filterData(self::$sql_data_cat, 'language', lang('#lang_all')[0]);
            ...
            self::$lines_prod = \eMarket\Func::filterData(self::$sql_data_prod, 'language', lang('#lang_all')[0]);
        } else {
            self::$sql_data_cat = \eMarket\Pdo::getColAssoc("SELECT * FROM " . TABLE_CATEGORIES . " WHERE parent_id=? ORDER BY sort_category DESC", [self::$parent_id]);
            self::$lines_cat = \eMarket\Func::filterData(self::$sql_data_cat, 'language', lang('#lang_all')[0]);
            self::$sql_data_prod = \eMarket\Pdo::getColAssoc("SELECT * FROM " . TABLE_PRODUCTS . " WHERE parent_id=? ORDER BY id DESC", [self::$parent_id]);
            self::$lines_prod = \eMarket\Func::filterData(self::$sql_data_prod, 'language', lang('#lang_all')[0]);
        }
        ...
        self::$arr_merge = \eMarket\Func::arrayMergeOriginKey('cat', 'prod', self::$lines_cat, self::$lines_prod);

        $navigate = \eMarket\Navigation::getLink(self::$count_lines_merge, \eMarket\Settings::linesOnPage(), 1);
                ...
                foreach (self::$sql_data_prod as $sql_modal_prod) {
                    ...
                    if ($sql_modal_prod['language'] == lang('#lang_all')[0] && $sql_modal_prod['id'] == $modal_id_prod) {
                        ...
                        if (self::$parent_id == 0) {
                            ...
                            $attributes_data[$modal_id_prod] = json_encode(\eMarket\Pdo::getColAssoc("SELECT attributes FROM " . TABLE_CATEGORIES . " WHERE id=? AND language=?", [self::$parent_id, lang('#lang_all')[0]])[0]['attributes']);
                        }
                    }
                }
