...
class Fileupload {
                    ...
                    jQuery.ajax({
                        ...
                        success: function (image_size) {
                            ...
                            if (this_height < quality_height && this_width < quality_width) {
                                ...
                                if ($('#add').val() === 'ok') {
                                    $('<div class="file-upload position-relative" id="image_add_new_' + hash_name + '"/>').html('<img src="/uploads/temp/thumbnail/' + file.name + '" class="img-thumbnail" id="general_' + hash_name + '" /><div class="block align-items-center justify-content-evenly"><button class="btn btn-primary btn-sm" type="button" name="deleteImageAddNew_' + hash_name + '" onclick="Fileupload.deleteImageAddNew(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-trash"></span></button> <button class="btn btn-primary btn-sm" type="button" name="imageGeneralAddNew_' + hash_name + '" onclick="Fileupload.imageGeneralAddNew(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-star"></span></button></div></div>').appendTo('#logo'); // Вставляем лого
                                }
                                if ($('#edit').val() !== '') {
    ...
    static getImageToEdit(logo_general_edit, logo_edit, modal_id, dir) {
        for (var x = 0; x < logo_edit[modal_id].length; x++) {
            ...
            $('<div class="file-upload position-relative" id="image_edit_' + x + '"/>').html('<img src="/uploads/images/' + dir + '/resize_0/' + image + '" class="img-thumbnail" id="general_' + x + '" /><div class="block align-items-center justify-content-evenly"><button class="btn btn-primary btn-sm" type="button" name="delete_image_' + x + '" onclick="Fileupload.deleteImageEdit(\'' + image + '\', \'' + x + '\')"><span class="bi-trash"></span></button> <button class="btn btn-primary btn-sm" type="button" name="image_general_edit' + x + '" onclick="Fileupload.imageGeneralEdit(\'' + image + '\', \'' + x + '\')"><span class="bi-star"></span></button></div></div>').appendTo('#logo');
