...
class FileuploadProduct {
                    ...
                    jQuery.ajax({
                        ...
                        success: function (image_size) {
                            ...
                            if (this_height < quality_height && this_width < quality_width) {
                                ...
                                if ($('#add_product').val() === 'ok') {
                                    $('<div class="file-upload" id="image_add_new_product_' + hash_name + '"/>').html('<img src="/uploads/temp/thumbnail/' + file.name + '?' + Math.random() + '" class="img-thumbnail" id="general_product_' + hash_name + '" /><div class="block"><button class="btn btn-primary btn-sm" type="button" name="deleteImageAddNewProduct_' + hash_name + '" onclick="FileuploadProduct.deleteImageAddNewProduct(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-trash"></span></button> <button class="btn btn-primary btn-sm" type="button" name="imageGeneralAddNewProduct_' + hash_name + '" onclick="FileuploadProduct.imageGeneralAddNewProduct(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-star"></span></button></div></div>').appendTo('#logo-product'); // Вставляем лого
                                }
                                if ($('#edit_product').val() !== '') {
                                    $('<div class="file-upload" id="image_edit_new_product_' + hash_name + '"/>').html('<img src="/uploads/temp/thumbnail/' + file.name + '?' + Math.random() + '" class="img-thumbnail" id="general_edit_product_' + hash_name + '" /><div class="block"><button class="btn btn-primary btn-sm" type="button" name="deleteImageEditNewProduct_' + hash_name + '" onclick="FileuploadProduct.deleteImageEditNewProduct(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-trash"></span></button> <button class="btn btn-primary btn-sm" type="button" name="imageGeneralEditNewProduct_' + hash_name + '" onclick="FileuploadProduct.imageGeneralEditNewProduct(\'' + file.name + '\', \'' + hash_name + '\')"><span class="bi-star"></span></button></div></div>').appendTo('#logo-product'); // Вставляем лого
                                }
                            }
                        }
                    });
    ...
    static getImageToEditProduct(logo_general_edit, logo_edit, modal_id, dir) {
        for (var x = 0; x < logo_edit[modal_id].length; x++) {
            ...
            $('<div class="file-upload" id="image_edit_product_' + x + '"/>').html('<img src="/uploads/images/' + dir + '/resize_0/' + image + '" class="img-thumbnail" id="general_product_' + x + '" /><div class="block"><button class="btn btn-primary btn-sm" type="button" name="delete_image_product_' + x + '" onclick="FileuploadProduct.deleteImageEditProduct(\'' + image + '\', \'' + x + '\')"><span class="bi-trash"></span></button> <button class="btn btn-primary btn-sm" type="button" name="image_general_edit_product' + x + '" onclick="FileuploadProduct.imageGeneralEditProduct(\'' + image + '\', \'' + x + '\')"><span class="bi-star"></span></button></div></div>').appendTo('#logo-product');
