...
// *************************************//
require($VALID->inSERVER('DOCUMENT_ROOT') . '/controller/admin/pages/settings/modal/articuls_edit.php');
?>

<!-- Модальное окно "Изменить" -->
<div id="articuls_edit<?php echo $lines[$k][0] ?>" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="form_articuls<?php echo $lines[$k][0] ?>" name="form_articuls<?php echo $lines[$k][0] ?>" action="javascript:void(null);" onsubmit="call_articuls<?php echo $lines[$k][0] ?>()" method="get" enctype="multipart/form-data">
                        ...
                        <div id="<?php echo $lang_all[0] . $lines[$k][0] ?>" class="tab-pane fade in active">
                            ...
                            <div class="form-group">
                                <div class="input-group has-error">
                                    ...
                                    <input class="input-sm form-control" type="text" name="articul_edit<?php echo $lang_all[0] ?>" id="articul_edit<?php echo $lang_all[0] ?>" value="<?php echo $value_edit[0] ?>" />
                                </div>
                            </div>
                        </div>
                            ...
                            for ($xl = 1; $xl < count($lang_all); $xl++) {
                                ...
                                <div id="<?php echo $lang_all[$xl] . $lines[$k][0] ?>" class="tab-pane fade">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group has-error">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-sort-by-order"></span></span>
                                            <input class="input-sm form-control" type="text" name="articul_edit<?php echo $lang_all[$xl] ?>" id="articul_edit<?php echo $lang_all[$xl] ?>" value="<?php echo $value_edit[$xl] ?>" />
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript" language="javascript">
    function call_articuls<?php echo $lines[$k][0] ?>() {
        var msg = $('#form_articuls<?php echo $lines[$k][0] ?>').serialize();
        $.ajax({
            type: 'GET',
            url: '/controller/admin/pages/settings/articuls.php',
            data: msg,
            success: function (data) {
                $('#articuls_add<?php echo $lines[$k][0] ?>').modal('hide');
